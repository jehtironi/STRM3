    ...
    while (1)
    {
        ...
        for (int i = 0; i < TOUCH_PAD_MAX; i++)
        {
            if (s_pad_activated[i] == true) //Verifica qual touch foi ativado
            {
                ...
                if (i == 4) //Alteração na temperatura
                {
...
void app_main(void)
{
    ...
    xTaskCreatePinnedToCore(&display, "display", 2048, NULL, 2, NULL, tskNO_AFFINITY); // inicia task para display
    xTaskCreatePinnedToCore(&read_task, "read_task", 2048, NULL, 17, NULL, tskNO_AFFINITY);
    xTaskCreatePinnedToCore(&sensorCintoMotorista, "sensorCintoMotorista", 2048, NULL, 11, NULL, tskNO_AFFINITY);
    xTaskCreatePinnedToCore(&sensorCintoPassageiro, "sensorCintoPassageiro", 2048, NULL, 10, NULL, tskNO_AFFINITY);
    xTaskCreatePinnedToCore(&sensorAbs, "sensorAbs", 2048, NULL, 8, NULL, tskNO_AFFINITY);
    xTaskCreatePinnedToCore(&sensorAibag, "sensorAibag", 2048, NULL, 15, NULL, tskNO_AFFINITY);
    xTaskCreatePinnedToCore(&sensorTemperatura, "sensorTemperatura", 2048, NULL, 3, NULL, tskNO_AFFINITY);
    xTaskCreatePinnedToCore(&sensorInjecao, "sensorInjecao", 2048, NULL, 14, NULL, tskNO_AFFINITY);
}
